...
Reducer::Reducer(
  ...
  TORCH_WARN(local_worker_);
  if (local_worker_ != -1){
    other_ranks_gradients.reserve(local_worker_-1);
    std::vector<std::vector<at::Tensor>> temp_tensors;
    for (int i = 0;i < local_worker_-1;i++){
      other_ranks_gradients.push_back(temp_tensors);
    }
  }
...
void Reducer::exchange_variable(VariableIndex index) {
  const auto rep_index = index.replica_index;
  //const auto var_index = index.variable_index;
  const auto& buc_index = variable_locators_[index.variable_index];
  auto& bucket = buckets_[buc_index.bucket_index];
  auto& replica = bucket.replicas[rep_index];
  auto& variable = replica.variables[buc_index.intra_bucket_index];
  const auto offset = replica.offsets[buc_index.intra_bucket_index];
  const auto length = replica.lengths[buc_index.intra_bucket_index];
  auto& bucket_view = replica.bucket_views_in[buc_index.intra_bucket_index];
  ...
  runGradCallbackForVariable(variable, [&](auto& grad) {
    if (grad.defined()) {
      ...
      if (!grad.is_alias_of(bucket_view)) {
        ...
        //if this is the last variable of current bucket
        if (buc_index.bucket_index != variable_locators_[index.variable_index-1].bucket_index){
          //TORCH_WARN(buc_index.bucket_index);
          std::vector<at::Tensor> bucket_tensors;
          bucket_tensors.reserve(bucket.replicas.size());
          for (const auto& rep : bucket.replicas) {
            bucket_tensors.push_back(rep.contents.clone());
          }
          //TORCH_WARN(num_iterations_);
          other_ranks_gradients[num_iterations_%local_worker_-1][buc_index.bucket_index]=bucket_tensors;
          //TORCH_WARN("rank ",num_iterations_%local_worker_-1,",bucket ",buc_index.bucket_index,":",at::sum(other_ranks_gradients[num_iterations_%local_worker_-1][buc_index.bucket_index][0]));
        }
        if (gradient_as_bucket_view_) {
...
void Reducer::autograd_hook(VariableIndex index) {
  ...
  //TORCH_WARN_ONCE(local_worker_);
	if (local_worker_ != -1 && num_iterations_ % local_worker_ != 0){
    //exchange gradients of other ranks into cpu memory
    
    exchange_variable(index);
    const auto& bucket_index = variable_locators_[index.variable_index];
    auto& bucket = buckets_[bucket_index.bucket_index];
    auto& replica = bucket.replicas[index.replica_index];
    if (--replica.pending == 0)
      if (--bucket.pending == 0) {
        auto f1 = c10::make_intrusive<c10::ivalue::Future>(c10::IntType::get());
        f1->markCompleted(1);
        bucket.future_work = f1;
...
void Reducer::mark_variable_ready(VariableIndex index) {
  ...
  if (should_rebuild_buckets() && (local_worker_ == -1 || num_iterations_ == local_worker_) ) {
    push_rebuilt_params(index);
  }
...
void Reducer::all_reduce_bucket(Bucket& bucket) {
  ...
  if (local_worker_ != -1){
    //TORCH_WARN("rank ",num_iterations_%local_worker_+1,", bucket ",next_bucket_,":",at::sum(tensors[0]));
    for (int i = 0;i < local_worker_ - 1;i++){
      tensors[0]=tensors[0]+other_ranks_gradients[i][next_bucket_][0];
      //TORCH_WARN("rank ",i,", bucket ",next_bucket_,":",at::sum(other_ranks_gradients[i][next_bucket_][0]));
    }
    tensors[0] /= local_worker_;
    //TORCH_WARN("average:",at::sum(tensors[0]));
  }
  else{
    //TORCH_WARN("rank ",process_group_->getRank(),":",at::sum(tensors[0]));
  }
  // See Note [DDP Communication Hook]
...
void Reducer::mark_bucket_ready(size_t bucket_index) {
  ...
  if (bucket_index > next_bucket_) {
    TORCH_WARN("bucket_index :",bucket_index," next_bucket_ :",next_bucket_);
    return;
  }
...
void Reducer::initialize_buckets(
  ...
  for (int i = 0;i < local_worker_ - 1;i++){
    other_ranks_gradients[i].clear();
    other_ranks_gradients[i].reserve(buckets_.size());
    std::vector<at::Tensor> temp_bucket;
    for (int j = 0;j < buckets_.size();j++){
      other_ranks_gradients[i].push_back(temp_bucket);
    }
  }
}
...
bool Reducer::rebuild_buckets() {
  ...
  if (!should_rebuild_buckets() || rebuilt_params_.empty() || (local_worker_ != -1 && num_iterations_ != local_worker_+1)) {
    return false;
  }
  ...
  TORCH_WARN_ONCE("rebuild", num_iterations_);
  return true;
}
