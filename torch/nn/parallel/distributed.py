...
class DistributedDataParallel(Module):
    ...
    def __init__(
        ...
        # Builds reducer.
        self.local_worker = -1
        #print('distributed:',max_worker)
        if max_worker != -1 and dist.group.WORLD.size() < max_worker:
            self.local_worker = int(max_worker/dist.group.WORLD.size())
        print('distributed:',self.local_worker)
    ...
    def _ddp_init_helper(self, parameters, expect_sparse_gradient, param_to_name_mapping):
        ...
        if self.local_worker != -1:
            assert (
                self.local_worker > 1
            ), "local worker number should be more than 1."
    ...
    def forward(self, *inputs, **kwargs):
        with torch.autograd.profiler.record_function("DistributedDataParallel.forward"):
            ...
            if torch.is_grad_enabled() and self.require_backward_grad_sync:
                ...
                # if self.local_worker > 1:
                #     seed = int(dist.get_rank()*self.local_worker+self.num_iterations%self.local_worker)
                #     random.seed(seed)
                #     os.environ['PYTHONHASHSEED'] = str(seed)  
                #     np.random.seed(seed)
                #     torch.manual_seed(seed)
                #     torch.cuda.manual_seed(seed)
                #     torch.cuda.manual_seed_all(seed) 
                #     #print('rank:',dist.get_rank(),' set seed:',seed,'  faaa')
